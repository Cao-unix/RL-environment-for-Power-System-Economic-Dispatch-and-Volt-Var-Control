...
if __name__ == "__main__":
    ...
    # PPO 的默认参数（把你原来的保留一部分，并加上一些 PPO 需要的）
    regular_parameters = {
        # Exploration（对 PPO 不太用得上，可以忽略）
        "start_timesteps": 0,
        "initial_eps": 0.0,
        "end_eps": 0.0,
        "eps_decay_period": 1,
        ...
        # Learning
        "discount": 0.95,
        "buffer_size": int(1e6),   # 现在其实不用 replay buffer 了
        "batch_size": 64,
        "optimizer": "Adam",
    ...
    interact_with_environment(
        env, num_actions, state_dim, device, args, parameters
    )
